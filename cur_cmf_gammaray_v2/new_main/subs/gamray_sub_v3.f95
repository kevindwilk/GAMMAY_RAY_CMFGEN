...
!
	...
	DO I=1,NF_GRID_PTS
	  ...
	  IF(NU_END(I) .EQ.1)THEN
	    WRITE(7,'(I5,2X,I5,2X,2(ES16.6,2X),A18,2(ES16.6,2X))')I,NU_END(I),NU_GRID_VEC(I),T1,' ',NU_GRID_VEC(NU_END(I)),NU_GRID_VEC(NU_END(I)+1)
	  ELSE IF(NU_END(I) .NE. 1 .AND. NU_END(I) .NE. NF_GRID_PTS)THEN
	    WRITE(7,'(I5,2X,I5,2X,5(ES16.6,2X))')I,NU_END(I),NU_GRID_VEC(I),T1,NU_GRID_VEC(NU_END(I)-1),NU_GRID_VEC(NU_END(I)),NU_GRID_VEC(NU_END(I)+1)
	  ELSE IF(I .EQ. NF_GRID_PTS .OR. NU_END(I) .EQ. NF_GRID_PTS)THEN
	    WRITE(7,'(I5,2X,I5,2X,4(ES16.6,2X),A18)')I,NU_END(I),NU_GRID_VEC(I),T1,NU_GRID_VEC(NU_END(I)-1),NU_GRID_VEC(NU_END(I)),' '
	  END IF
	END DO
	CLOSE(UNIT=7)
!
...
!
	CALL GAMRAY_ENERGY_DEPOSIT_V6(GAM_I,GAM_ETA_SCAT,E_SCAT_ARRAY,CHI_ARRAY_ABS, &
