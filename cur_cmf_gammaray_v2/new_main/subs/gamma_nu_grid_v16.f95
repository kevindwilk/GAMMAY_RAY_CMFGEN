...
!-----------------------------------------------------------------------------------------------
!
	SUBROUTINE NU_GRID_V16(N_GAMMA,NU_VEC,NU_GRID_VEC,NF_GRID_PTS)
...
!
	...
	IF(T2*PLANCK .LT. EGAM_MIN)THEN
	  IF(T1*PLANCK .GT. EGAM_MIN .AND. &
	      T2*10.0D0*PLANCK .GT. EGAM_MIN )THEN
	    NU_MIN=EGAM_MIN/PLANCK
	  ELSE IF(NU_VEC(NGAM)*PLANCK .GT. EGAM_MIN .AND. &
...
!
		...
		INU=INU+1
		NU_GRID_VEC(INU)=NU_NEW
		IF(GRID_TST)CALL TST_GRID(IGAM,NGAM,INU,NU_GRID_MAX,STRING,LUER)
...
!
	...
	END
!
	SUBROUTINE TST_GRID(IGAM,NGAM,INU,NMAX,STRING,LUER)
	IMPLICIT NONE
!
	INTEGER :: IGAM
	INTEGER :: NGAM
...
!
	WRITE(LUER,'(A,I3,A,I3)')'Line ',IGAM,' / ',NGAM
	WRITE(LUER,'(A)')TRIM(STRING)
	WRITE(LUER,'(A,I5,A,I5)')'Frequency indx ',INU,' / ',NMAX
!
	RETURN
	END SUBROUTINE
