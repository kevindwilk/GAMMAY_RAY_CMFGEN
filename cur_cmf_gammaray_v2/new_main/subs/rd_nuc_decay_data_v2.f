	SUBROUTINE RD_NUC_DECAY_DATA_V2(INCL_RAD_DECAYS,GAMRAY_TRANS,ND,LU)
	USE MOD_CMFGEN
	USE NUC_ISO_MOD
	IMPLICIT NONE
!
! Altered 10-Oct-2017 : Added ISO(:)%NUM_DECAYS and ISO(:)%DECAY_LUM to routine for gamma-ray
!			routine
!
...
!
	...
	ELSE
	  ...
	  IF(GAMRAY_TRANS .EQ. 'ABS_TRANS')THEN
	    WRITE(6,*)' '
	    WRITE(6,*)'Warning -- NUC_DECAY_DATA has the old format'
	    WRITE(6,*)'This will affect results if usig ABS_TRANS option for gamma-ray transport'
	    WRITE(6,*)'Please add the kinetic energy to the decay chain table'
	    STOP
	  END IF
	END IF
!
	  ...
	  DO WHILE(NUM_DECAY_PATHS .EQ. 0)
	    ...
	    ELSE IF(INDEX(STRING,'!Total number of isotopes') .NE. 0)THEN
	      ...
	      IF(NUM_ISOTOPES .GT. MAX_NUM_ISOTOPES)THEN
	        WRITE(LUER,*)'Error in RD_NUCLEAR_DECAY_DATA_V2- MAX_NUM_ISOTOPES in NUC_IS_MOD too small'
	    ...
	    ELSE IF(INDEX(STRING,'!Number of reactions') .NE. 0)THEN
	      ...
	      IF(NUM_DECAY_PATHS.GT. MAX_NUM_DECAY_PATHS)THEN
	        WRITE(LUER,*)'Error in RD_NUCLEAR_DECAY_DATA_V2- MAX_NUM_DECAY_PATHS in NUC_IS_MOD too small'
