...
!------------------------------------------------------------------------------------
!------------------------------------------------------------------------------------
	SUBROUTINE ETA_SCAT_V5_BETA(NF_GRID_PTS,NU_GRID_VEC,IN_NU,NU_END, &
...
!
!*******************************************************************************************
! Maybe add the piece that contributes from NU_BACK
!*******************************************************************************************
!
      IF(DO_NORM_ETA)THEN
	DO DI=1,ND
	  ...
	  T2=COMPTON_SIG(T1)
	  I_NORM(DI)=I_NORM(DI)*DELTA_NU*ED(DI)*T2/T1
...
!
	...
	REAL*8, PARAMETER :: A  =7.297355257D-3      ! Fine structure constant
!
	PI=ACOS(-ONE)
	R_E=(A*R_C)/(TWO*PI)
	R_E2=R_E*R_E
!
	...
	EPSI=ONE+TWO*X
!
	COMPTON_SIG=TWO*PI*( ((ONE+X)/X/X/X)*( (TWO*X*(ONE+X))/EPSI-LOG(EPSI) ) &
