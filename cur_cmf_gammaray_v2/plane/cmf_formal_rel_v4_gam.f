...
!
	END MODULE EXT_REL_GRID_V4_GAM
!
	SUBROUTINE CMF_FORMAL_REL_V4_GAM
	...
	1	     NA,NU_INDX,GAM_ETA,GAM_INT,ANG_MULT)
        USE EXT_REL_GRID_V4_GAM
...
!
	...
	REAL*8 P(NP)
	REAL*8 ETA(ND)	
	REAL*8 GAM_ETA(ND,NA)
	REAL*8 GAM_INT(ND,NA)
	REAL*8 CHI(ND)
	REAL*8 ESEC(ND)
	REAL*8 VDOP_VEC(ND)
	REAL*8 VDOP_FRAC
...
!
	...
	LOGICAL NEW_R_GRID
	REAL*8, DIMENSION(:), ALLOCATABLE :: ZRAY
	REAL*8, DIMENSION(:), ALLOCATABLE :: TEMPINT_M,TEMPINT_P
	INTEGER :: KDW,KDW2
!
!
!
...
!            CALL SOLVE_CMF_FORMAL_V2(CHI_RAY,ETA_RAY,IP,FREQ,NU_ON_dNU,INNER_BND_METH,b_planck,dBdTAU,NRAY,NP,NC)
            CALL SOLVE_CMF_FORMAL_V3_GAM(CHI_RAY,GAM_ETA,IP,FREQ,NU_ON_dNU,INNER_BND_METH,b_planck,dBdTAU,NRAY,ND,NP,
	1				NC,NA,NU_INDX,GAM_INT)
	  END DO
!$OMP END PARALLEL DO
!
	ELSE 
